...
/*Buscamos archivos en el directorio Entradas*/
	...
	if(!empty($archivos)){
		...
		foreach ($cargas as $carga) {
			...
			$query = "LOAD DATA LOCAL INFILE '" . $path."/".$carga . "' INTO TABLE TABDIR_NOR FIELDS TERMINATED BY ';' LINES TERMINATED BY '\n' (RUT_CLIENTE, ID_DIRECCION, TIPO, CALLE, NUMERO, COD_COMUNA, BLOCK, PISO, DEPTO, POB_VILLA, COD_CIUDAD, ESTADO_DIR, OBSER1, DIR_ASIGNACION, SECTOR, DATOS_COMPLE, LATITUD, LONGITUD, FECHA_PROCESO, HORA_PROCESO, NOM_ARCHIVO, NUM_SECTOR, VALIDA_DIR, NEW_SECTOR, TIPO_ARCHIVO, FECHA_ORIG_ARCH)";
